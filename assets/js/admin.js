...
async function fetchLive(action) {
  ...
  /* ── ROOT CAUSE FIX ─────────────────────────────────────────────
     The GAS endpoint was returning HTTP 403 "Host not in allowlist"
     because the deployment was restricted. The old code never read
     the 403 body so no error was ever shown — just "Connecting…"
     forever. Now we detect 403 and surface the exact fix needed.

     PERMANENT FIX (required in Google Apps Script):
       Deploy → Manage Deployments → Edit (✏) →
       set "Who has access" to "Anyone" → Deploy as a NEW version.
  ─────────────────────────────────────────────────────────────── */
  ...
  try {
    ...
    /* ── 403: deployment access not set to "Anyone" ─────────────── */
    if (res.status === 403) {
      const body = await res.text().catch(() => "");
      if (body.includes("allowlist") || body.includes("not in")) {
        throw new Error(
          "403 — Apps Script deployment is restricted to specific hosts. " +
          "FIX: Go to script.google.com → your project → Deploy → Manage Deployments → " +
          "Edit (pencil) → set \'Who has access\' to \'Anyone\' → click Deploy (new version)."
        );
      }
      throw new Error(`403 Forbidden from Apps Script. Re-deploy with \'Who has access\' set to \'Anyone\'.`);
    }

    if (!res.ok) throw new Error(`HTTP ${res.status} from Apps Script — check deployment settings.`);

    /* ── HTML error page instead of JSON ────────────────────────── */
    const contentType = res.headers.get("content-type") || "";
    if (!contentType.includes("json") && !contentType.includes("javascript")) {
      throw new Error(
        "Apps Script returned HTML instead of JSON — the deployment is outdated. " +
        "Re-deploy your script as a new version."
      );
    }
    ...
    if (err.name === "AbortError") {
      throw new Error(
        `Timed out after ${TIMEOUT_MS / 1000}s. Verify your Apps Script URL is correct and ` +
        `deployed with \'Who has access\' set to \'Anyone\'.`
      );
    }
    /* TypeError / NetworkError = CORS block */
    if (err.name === "TypeError" || err.message.includes("Failed to fetch")) {
      throw new Error(
        "Browser blocked the request (CORS/network error). " +
        "In Apps Script → Deploy → Manage Deployments → Edit → " +
        "set \'Who has access\' to \'Anyone\' → deploy as a NEW version."
      );
    }
