...
async function fetchLive(action) {
  ...
  /* ── FIX: Google Apps Script Web Apps issue a 302 redirect to
     script.googleusercontent.com. Browsers block this redirect with
     a CORS error when the request carries an Origin header, causing
     fetch() to throw a TypeError that is never caught here — so the
     dashboard gets stuck on "Connecting…" forever.

     Solution: use a JSONP-style approach via a <script> tag which
     bypasses CORS entirely, with a Promise wrapper and hard timeout.
     Falls back to a plain fetch (works when GAS has CORS headers set
     correctly or is accessed from the same domain). ─────────────── */

  const TIMEOUT_MS = 15000;

  /* Helper: JSONP fetch — injects a <script> tag, GAS must support
     ?callback=xxx  (add  output ContentService.MimeType.JAVASCRIPT
     and wrap with callback in your doGet if you use this path).
     We try plain fetch first; JSONP is the fallback. */
  function fetchViaScript(url) {
    return new Promise((resolve, reject) => {
      const cbName = `_rsGasCb_${Date.now()}`;
      const script = document.createElement("script");
      const timer  = setTimeout(() => {
        cleanup();
        reject(new Error("JSONP request timed out. Check your Apps Script deployment URL."));
      }, TIMEOUT_MS);
      ...
      script.onerror = () => { cleanup(); reject(new Error("Script load failed — Apps Script URL may be incorrect or the deployment is not public.")); };
  ...
  /* ── Attempt 1: plain fetch with redirect:follow ─────────────── */
  try {
    ...
    /* Detect HTML error page — old GAS version without doGet() returns HTML
       instead of JSON, causing a SyntaxError that was previously swallowed,
       leaving the dashboard stuck on "Connecting…" forever. */
    const contentType = res.headers.get("content-type") || "";
    if (!contentType.includes("json") && !contentType.includes("javascript")) {
      throw new Error(
        "Apps Script returned an HTML page instead of JSON. " +
        "The deployed version may be outdated — re-deploy your Apps Script as a new version."
      );
    }
    ...
    catch { throw new Error("Could not parse Apps Script response — re-deploy the latest version."); }
    ...
    /* ── Attempt 2: CORS blocked (TypeError: Failed to fetch) ─────
       This is the #1 cause of the permanent "Connecting…" bug.
       GAS redirects to a different origin and the browser blocks it.
       Try JSONP as a fallback — requires ?callback= support in doGet. */
    if (err.name === "AbortError") {
      throw new Error(`Request timed out after ${TIMEOUT_MS / 1000}s — verify your Apps Script is deployed as "Execute as Me" and "Anyone" can access it.`);
    }

    if (err.name === "TypeError" || err.message.includes("Failed to fetch") || err.message.includes("NetworkError")) {
      console.warn("RS Admin: CORS/redirect blocked on plain fetch — trying JSONP fallback…", err.message);
      try {
        const data = await fetchViaScript(url);
        if (data && data.status === "error") throw new Error(data.message || "Apps Script returned an error");
        return data;
      } catch (jsonpErr) {
        /* Both methods failed — surface a clear, actionable error */
        throw new Error(
          `CORS error: browser blocked the request to Google Apps Script. ` +
          `Fix: In your Apps Script deployment settings, set "Who has access" to "Anyone" (not "Anyone with Google account"). ` +
          `Original error: ${jsonpErr.message}`
        );
      }
    }
...
/* ═══════════════════════════════════════════════════════════════
   NOTIFICATION BADGE — live count of new + pending items
   Runs on every admin page so the bell always shows real data
   ═══════════════════════════════════════════════════════════════ */
async function refreshNotifBadge() {
  ...
  try {
    ...
    badge.textContent   = alerts > 0 ? alerts : "0";
    badge.style.display = "";
    /* Pulse the bell if there are urgent items */
    const bell = document.getElementById("notif-bell");
    if (bell && alerts > 0) bell.style.color = "var(--gold)";
  } catch (e) {
    badge.textContent = "!";  /* Show ! if GAS unreachable */
  }
}
...
function setConnectionStatus(state, extra) {
  ...
  const cfg = {
    ...
    error:     { cls: "cs-error",     dot: "●", label: "Cannot reach Google Sheets — check deployment access (must be "Anyone") then click Refresh" },
...
function updateSidebarBadges(data) {
  ...
  /* Update both by ID and by class so every page is covered */
  const notifEl = document.getElementById("notif-badge") || document.querySelector(".notif-badge");
  const bell    = document.getElementById("notif-bell");
  ...
  if (notifEl) {
    const alerts = (data.newRequests || 0) + (data.pendingFollowUps || 0);
    notifEl.textContent   = alerts > 0 ? alerts : "0";
    notifEl.style.display = "";
    if (bell && alerts > 0) bell.style.color = "var(--gold)";
    else if (bell)           bell.style.color = "";
  }
}
...
async function loadDashboard() {
  ...
  try {
    ...
    console.error("RS Admin: dashboard fetch failed →", err.message);
    setConnectionStatus("error");
    /* Show the real error in the activity feed so it's visible in the UI */
    const feed = document.getElementById("activity-feed");
    if (feed) {
      feed.innerHTML = `<div class="activity-empty" style="color:#f87171;">
...
function _adminBoot() {
  ...
  /* Refresh notification bell on every admin page (non-blocking) */
  if (page !== "dashboard.html") refreshNotifBadge();
