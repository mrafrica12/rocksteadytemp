...
async function fetchLive(action) {
  ...
  try {
    ...
    /* Detect HTML error page — old GAS version without doGet() returns HTML,
       not JSON. This causes res.json() to throw a SyntaxError which browsers
       sometimes swallow silently due to the CORS redirect chain, leaving the
       dashboard stuck on "Connecting…" forever. */
    const contentType = res.headers.get("content-type") || "";
    if (!contentType.includes("json") && !contentType.includes("javascript")) {
      throw new Error(
        "Apps Script returned an HTML page instead of JSON. " +
        "The deployed version is outdated — paste the new google-apps-script.js " +
        "into your Apps Script editor and deploy a new version."
      );
    }
    ...
    try {
      ...
      throw new Error(
        ...
        "Deploy the latest google-apps-script.js as a new version."
      );
    }
    ...
    if (err.name === "AbortError") {
      throw new Error("Request timed out after 20s — Apps Script may be on an old version without doGet(). Deploy the new version.");
    }
    throw err;
  }
}
...
function setConnectionStatus(state, extra) {
  ...
  const cfg = {
    ...
    error:     { cls: "cs-error",     dot: "●", label: "Could not reach Google Sheets — deploy new GAS version then Refresh" },
